...
// Definer denne typen lokalt i stedet for å importere intern type
...
export function MFASetup({ onStatusChange }: MFASetupProps) {
  ...
  const startEnrollment = async () => {
    if (!friendlyName.trim()) {
      setError('Velg et navn for autentikasjonen');
      return;
    }
    ...
    try {
      ...
      const { data, error } = await supabase.auth.mfa.enroll(enrollParams);
      ...
      setError(err instanceof Error ? err.message : '2FA Feilet!');
      setStep('name');
    } finally {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Legg til et ekstra beskyttelses lager
        </CardDescription>
      </CardHeader>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Lagt til {new Date(factor.created_at).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                <button
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="e.g., Jobb Telefon, Min Iphone"
                autoFocus
              />
              <p className="text-sm text-gray-500">
                Gi autentiseringen et navn for å finne fram lettere
              </p>
            </div>
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {actionInProgress ? 'Processing...' : 'Fortsett'}
              </button>
            </div>
          </div>
        )}
