...
import supabase from '@/lib/supabase/client'; // <-- BYTTET HER!
...
function CreateTaskDialog({ onTaskCreated }: CreateTaskDialogProps) {
    ...
    const handleAddTask = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            setLoading(true);
            // BRUK raw supabase direkte, ingen .createTask
...
export default function TaskManagementPage() {
    ...
    const loadTasks = async (): Promise<void> => {
        try {
            ...
            // BRUK raw supabase direkte, ingen .getMyTodoList
            let query = supabase
                .from('todo_list')
                .select('*')
                .eq('owner', user!.id)
    ...
    const handleRemoveTask = async (id: number): Promise<void> => {
        try {
            // BRUK raw supabase direkte, ingen .removeTask
            const { error: supabaseError } = await supabase
                .from('todo_list')
                .delete()
                .eq('id', id);
    ...
    const handleMarkAsDone = async (id: number): Promise<void> => {
        try {
            // BRUK raw supabase direkte, ingen .updateAsDone
            const { error: supabaseError } = await supabase
                .from('todo_list')
                .update({ done: true })
                .eq('id', id);
