...
export default function FileManagementPage() {
    ...
    // Hent filer fra Supabase storage-bucket
    const loadFiles = async () => {
        try {
            setLoading(true);
            setError('');
            const { data, error } = await supabase.storage.from('user-files').list(user!.id + '/', { limit: 100 });
    ...
    const handleShare = async (filename: string) => {
        try {
            setError('');
            const path = `${user!.id}/${filename.split('/').pop()}`;
            const { data, error } = await supabase.storage.from('user-files').createSignedUrl(path, 60 * 60 * 24); // 24t
            if (error || !data) throw error;
    ...
    const handleDelete = async () => {
        ...
        try {
            setError('');
            const path = `${user!.id}/${fileToDelete.split('/').pop()}`;
